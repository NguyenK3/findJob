...
const ResumeTableByCompanyId = () => {
    ...
    const fetchResumes = async (page: number, rowsPerPage: number) => {
        try {
            var companyId = '';
            if (session?.user._id) {
                const user = await fetchUserById(session.user._id);
                if (user.company?._id) {
    ...
    const columns: GridColDef[] = [
        ...
        // { field: 'url', headerName: 'Resume URL', flex: 2 },
...
export default ResumeTableByCompanyId;