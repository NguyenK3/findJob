...
const CompanyListJob: React.FC = () => {
  ...
  const [sizeObject, setSizeObject] = useState(0)
  ...
  useEffect(() => {
    const loadJobs = async () => {
      ...
      // const data = await fetchCompanies(user.company?._id || '');
      // console.log("data", data)
      // console.log("user", user.company?._id);
      const { jobs, total } = await fetchJobs(
        paginationModel.page + 1,
        paginationModel.pageSize,
        searchSkills,
        searchSalary ?? undefined,
        user.company?._id ?? undefined as string | undefined,
        isActive,
    ...
    const loadCompanies = async () => {
      try {
        ...
        if (data) {
          // // console.log("data", data);
          // const obj = data
          // // console.log("test length", Object.keys(obj).length)
          // setSizeObject(Object.keys(obj).length)
          // console.log(data)
          // setCompanies(data);
          // const obj_1 = data.result // Đây là nhiều json
          // const obj_2 = data
          // console.log("test length", Object.keys(obj_2).length)
          // if ()
          let obj = data
  ...
  const handleCreateJob = async (job: IJob) => {
    try {
      ...
      const data = await fetchJobs(
        ...
        paginationModel.pageSize,
        searchSkills,
        user.company?._id || undefined as any,
        searchSalary || undefined as any,
      );
      setJobs(data.jobs);
      setOpenModal(false);
      setSnackbar({
